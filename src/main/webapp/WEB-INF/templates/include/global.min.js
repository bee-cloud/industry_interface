$(document).bind("ajaxSend", function (d, f) {
    var e = _gtag.traceId + ":" + (++_gtag.ajaxCount);
    f.setRequestHeader("x-trace-id", e)
});
...
Date.prototype.Format = function (e) {
    ...
    for (var d in f) {
        if (new RegExp("(" + d + ")").test(e)) {
            e = e.replace(RegExp.$1, (RegExp.$1.length == 1) ? (f[d]) : (("00" + f[d]).substr(("" + f[d]).length)))
        }
    }
    return e
};
$("#selectTime").click(function () {
    ...
    if ($("#timePicker").hasClass("hide")) {
        if (d != null && d.length != 0) {
            $("#startTime").attr("value", d.from);
            $("#endTime").attr("value", d.to);
            $("a").each(function () {
                if (this.text == c) {
...
function GetDateStr(i) {
    ...
    var g = d.getMonth() + 1;
    var j = d.getDate();
    return new Date(d).Format("yyyy-MM-dd") + " 00:00:00"
}
...
function getWeekLast() {
    var b = new Date();
    b.setDate(b.getDate() - b.getDay() + 8);
    return new Date(b).Format("yyyy-MM-dd") + " 00:00:00"
}
function getLastWeekFirst() {
    var b = new Date();
    b.setDate(b.getDate() - b.getDay() - 6);
    return new Date(b).Format("yyyy-MM-dd") + " 00:00:00"
}
function getLastWeekLast() {
    var b = new Date();
    b.setDate(b.getDate() - b.getDay());
    return new Date(b).Format("yyyy-MM-dd") + " 23:59:59"
}
...
function getTwoYear() {
    var b = new Date();
    b.setYear(b.getFullYear() - 2);
    return new Date(b).Format("yyyy-MM-dd HH:mm:ss")
}
function getFiveYear() {
    var b = new Date();
    b.setYear(b.getFullYear() - 5);
    return new Date(b).Format("yyyy-MM-dd HH:mm:ss")
}
$("#quickRanges a").click(function () {
    var b = mapOptions[this.text];
    if (b != null && b.length != 0) {
        request(this.text, b.start, b.end)
    }
});
...
function verify(g, f) {
    var e = g.match(/^\d{4}-(?:0\d|1[0-2])-(?:[0-2]\d|3[01])( (?:[01]\d|2[0-3])\:[0-5]\d\:[0-5]\d)?$/);
    var h = f.match(/^\d{4}-(?:0\d|1[0-2])-(?:[0-2]\d|3[01])( (?:[01]\d|2[0-3])\:[0-5]\d\:[0-5]\d)?$/);
    if (e == null || h == null) {
